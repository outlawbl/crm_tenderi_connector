...
def readPdf(file_location):
    ...
    # Defaultne vrijednosti, dodjeljeno adminu i vrijednost nije 0
    osnovni_podaci = {
        'assignedUserId': '1',
    ...
    for j in niz_redova:
        ...
        if j.startswith('II 4.a.'):
            ...
            if niz_redova[ind+4][0].isdigit():
                ...
                otvaranje = datetime.strptime(niz_redova[ind+5], '%d.%m.%Y. %H:%M')
            elif niz_redova[ind+6][0].isdigit():
                otvaranje = datetime.strptime(niz_redova[ind+6], '%d.%m.%Y. %H:%M')
