...
def main_function(pdf_data, file_path):
    print(datetime.now().strftime("%H:%M:%S"), 'Checking if tender already exists...')
    ...
    if total_tenders == 0:
        if pdf_data['tip_dokumenta'] == '3':
            print(datetime.now().strftime("%H:%M:%S"), 'Tender does not exists, new tender will be added...')
            ...
            def post_tender(pdf_data):
                ...
                accountsIds = [account_id]
                data['accountsIds'] = accountsIds
        ...
        print(datetime.now().strftime("%H:%M:%S"), 'Tender already exists...')